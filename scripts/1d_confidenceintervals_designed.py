...
def gen_model(X, type):
    ...
    if type == '3part-2':
        y = -np.sin(X)
        y[X < -np.pi / 2] = 2 * X[X < -np.pi / 2] + np.pi + 1
        y[X > 2] = -X[X > 2] + 1
    if type == '2part':
        y = X * 0
        y[X < 0] = 0.1 * X[X < 0]
...
def gen_data(n, type, eps):
    if Xdist == 'U':
        rng = ss.uniform(loc=-4, scale=8)
    if Xdist == 'N':
        rng = ss.norm()
    X = rng.rvs(size=(n, 1))
    y = gen_model(X=X, type=type)
    if eps>0:
...
def run_experiment(args):
    ...
    alpha = 0.05
    filename_base = f'{OUTPUT_DIR}/{method_label}_1d_n={n}_type={type}_eps={eps:.4f}_{substitution_policy}_X{Xdist}'
    filename_csv = f'{filename_base}.csv'
    filename_pickle = f'{filename_base}.pickle'
    
    X_pred = mesh_pts
    y_true = gen_model(X_pred, type)

    #generate one sample to set method parameters
    X, y = gen_data(n=n, type=type, eps=eps)
...
parser.add_argument("--type", choices=['kink', '3part-1', '3part-2', '2part'], required=True, help="1d model")

...
def get_svr_params(x, y):
    param_grid = {
        "C": [0.1, 1, 10, 100, 300, 500, 750, 1000, 1500, 2000, 3000],
        "degree": [1, 2, 3, 4],
...
def run_method(method, X, y, mcloops=1, test_size=0.2, bmr_M=10):
    ...
    for loop in range(mcloops):
        ...
        # build models
        if method == "rf":
            score, mape, pred, model = get_rf_model(
                X_train_scaled, y_train_scaled, X_test_scaled, y_test_scaled, params, scaler=std_scaler_y
            )
        if method == "svr":
            score, mape, pred, model = get_svr_model(
                X_train_scaled, y_train_scaled, X_test_scaled, y_test_scaled, params, scaler=std_scaler_y
            )
        if method == "mars":
            score, mape, pred, model = get_mars_model(
                X_train_scaled, y_train_scaled, X_test_scaled, y_test_scaled, params, scaler=std_scaler_y
            )
        if method == "catboost":
            score, mape, pred, model = get_catboost_model(
                X_train_scaled, y_train_scaled, X_test_scaled, y_test_scaled, scaler=std_scaler_y
            )
        if method == "bmr1":
            score, mape, pred, model = get_bmr_model(
                X_train_scaled,
                y_train_scaled,
                X_test_scaled,
                y_test_scaled,
...
def run_all(X, y, filename, mcloops=10):
    ...
    for method in ["bmr1", "bmr2", "bmr3", "bmr4", "rf", "svr", "mars", "catboost"]:
        results_all[method] = run_method(method=method, X=X, y=y, mcloops=mcloops, test_size=0.2, bmr_M=10)

...
def get_bmr_params(x, y, M, degree):
    ...
    n_trial = x.shape[0]
    param_grid = {"epsilon": [epsilon_trial * t for t in [0.5, 0.8, 1.0, 1.5, 2.0, 2.5, 3.0]],
...
def run_methods(X_train, y_train, X_test, y_test, std_scaler_y):
    ...
    score_bmr1, mape_bmr1, pred_bmr1, model_bmr1 = get_bmr_model(X_train, y_train, X_test, y_test, M=bmr_M, degree=1,
                                                             params=bmr1_params, scaler=std_scaler_y)
    # run bmr: deg2
    bmr2_params = get_bmr_params(X_train, y_train, M=bmr_M, degree=2)
    score_bmr2, mape_bmr2, pred_bmr2, model_bmr2 = get_bmr_model(X_train, y_train, X_test, y_test, M=bmr_M, degree=2,
                                                             params=bmr2_params, scaler=std_scaler_y)

    # run randomforest
    rf_params = get_rf_params(X_train, y_train)
    score_rf, mape_rf, pred_rf, model_rf = get_rf_model(X_train, y_train, X_test, y_test,
                                                         params=rf_params, scaler=std_scaler_y)

    # run SVR
    svr_params = get_svr_params(X_train, np.ravel(y_train))
    score_svr, mape_svr, pred_svr, model_svr = get_svr_model(X_train, y_train, X_test, y_test,
                                                              params=svr_params, scaler=std_scaler_y)

    # run MARS
    mars_params = get_mars_params(X_train, y_train)
    score_mars, mape_mars, pred_mars, model_mars = get_mars_model(X_train, y_train, X_test, y_test,
                                                                  params=mars_params, scaler=std_scaler_y)

    # run CatBoost
    score_cb, mape_cb, pred_cb, model_cb = get_catboost_model(X_train, y_train, X_test, y_test,
                                                              scaler=std_scaler_y)

    return [score_bmr1, score_bmr2, score_rf, score_svr, score_mars, score_cb], \
           [mape_bmr1, mape_bmr2, mape_rf, mape_svr, mape_mars, mape_cb], \
           [model_bmr1, model_bmr2, model_rf, model_svr, model_mars, model_cb]
